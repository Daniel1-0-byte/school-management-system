...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[v0][SCHOOL-REQUESTS] GET failed:', error instanceof Error ? error.message : 'Unknown error');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'reject') {
      ...
      // Get school admin email for approval notification
      ...
      if (schoolRequest.school_id) {
        try {
          ...
          console.error('[v0][PROVISION] Failed:', provisionError instanceof Error ? provisionError.message : 'Unknown error');
        }
      }
    ...
    console.error('[v0][SCHOOL-REQUESTS] POST failed:', error instanceof Error ? error.message : 'Unknown error');
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
