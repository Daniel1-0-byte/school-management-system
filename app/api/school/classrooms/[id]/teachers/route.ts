...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: stream, error: streamError } = await supabase.from('school_class_streams').select('id, name, school_class_id, academic_year_id').eq('id', id).eq('school_id', schoolId).single();
  ...
  const [{ data: assignments, error: assignmentError }, { data: teachers, error: teacherError }] = await Promise.all([
    queryTeacherAssignments().select('id, teacher_id, academic_year_id, subjects, is_primary, start_date, end_date').eq('school_id', schoolId).eq('class_id', stream.school_class_id).eq('academic_year_id', stream.academic_year_id).order('is_primary', { ascending: false }).order('created_at', { ascending: true }),
