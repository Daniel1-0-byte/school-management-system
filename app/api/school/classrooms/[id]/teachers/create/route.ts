...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('teacher_assignments').insert({ school_id: schoolId, class_id: stream.school_class_id, teacher_id: body.teacher_id, academic_year_id: body.academic_year_id, subjects: body.subjects || [], is_primary_teacher: body.is_primary_teacher, start_date: body.start_date || new Date().toISOString().slice(0, 10), end_date: body.end_date || null }).select().single();
