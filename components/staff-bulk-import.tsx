...
interface StaffBulkImportProps {
  schoolId: string;
  onSuccess?: (count: number) => void;
  onClose?: () => void;
}

export function StaffBulkImport({ schoolId, onSuccess, onClose }: StaffBulkImportProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [previewing, setPreviewing] = useState(false);
  const [previewData, setPreviewData] = useState<Partial<Staff>[]>([]);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const staff = parseFile(text, selectedFile.name);
      setPreviewData(staff.slice(0, 5));
      setPreviewing(true);
    } catch (err) {
  ...
  const parseCSV = (content: string): Partial<Staff>[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      staff.push(mapRecordToStaff(record));
    }
  ...
  const parseJSON = (content: string): Partial<Staff>[] => {
    try {
      const data = JSON.parse(content);
      if (!Array.isArray(data)) throw new Error('JSON must be an array');
      return data.map(mapRecordToStaff);
    } catch (err) {
      throw new Error('Invalid JSON format');
    }
  };

  const mapRecordToStaff = (record: Record<string, any>): Partial<Staff> => {
    return {
      ...
      experienceYears: record.experience_years ? parseInt(record.experience_years) : record.experienceYears,
      dateOfJoining: record.date_of_joining || record.dateOfJoining,
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const staff = parseFile(text, file.name);

      const importResult = await SchoolService.bulkCreateStaff(schoolId, staff);
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className={`text-sm ${result.errors.length === 0 ? 'text-green-600/80' : 'text-yellow-600/80'}`}>
                {result.created} staff members imported successfully
              ...
              <tbody>
                {previewData.map((s, i) => (
                  <tr key={i} className="border-b">
                    <td className="py-2 px-3">{s.firstName}</td>
                    <td className="py-2 px-3">{s.lastName}</td>
                    <td className="py-2 px-3 text-xs">{s.email}</td>
                    <td className="py-2 px-3">{StaffTransformer.roleLabel(s.role || 'teacher')}</td>
          ...
          >
            {importing ? (
              ...
              <>
                ...
                <span>Import {previewData.length} Staff</span>
              </>
            )}
          </button>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Expected columns: first_name, last_name, email, phone, role, department, qualification, experience_years, date_of_joining, status
      </p>
    </div>
  );
}
